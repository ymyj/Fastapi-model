...
class SoilPollutionSimulator:
    ...
    def setup_source(self):
        ...
        self.circle_source_mask = ((X_idx - center_x) ** 2) / radius_x ** 2 + (
                    (Y_idx - center_y) ** 2) / radius_y ** 2 <= 1
    ...
    def solve_step(self, c, current_step):
        ...
        c_new[1:-1, 1:-1] = c[1:-1, 1:-1] + (self.dt / self.retardation[1:-1, 1:-1]) * (
                    diff_x + diff_y + adv_x + adv_y + decay)
    ...
    def render_frame(self, c, step):
        ...
        ax.set_yticklabels([str(400 - y) for y in yticks])
    ...
    def calculate_stats(self):
        max_concentration = float(np.max(self.c))

        threshold = self.params['source_concentration'] * 0.005
        exceed_mask = self.c > threshold
        exceed_area = float(np.sum(exceed_mask)) * self.dx * self.dy

        exceed_volume = exceed_area * 1.0

        if max_concentration > threshold:
            rows, cols = np.where(self.c > threshold)

            migration_horizontal = float(np.max(cols) - np.min(cols)) * self.dx
            migration_vertical = float(np.max(rows) - np.min(rows)) * self.dy
            migration_distance = float(np.sqrt(migration_horizontal ** 2 + migration_vertical ** 2))

            source_x = int(self.params['source_x'])
            migration_vertical = float(np.max(rows)) * self.dy
        else:
            migration_distance = 0.0

        total_mass = float(np.sum(self.c)) * self.dx * self.dy

        return {
            'max_concentration': round(max_concentration, 2),
            'exceed_area': round(exceed_area, 0),
            'exceed_volume': round(exceed_volume, 0),
            'migration_distance': round(migration_distance, 1)
        }

    def get_depth_profile(self, x_index=None):
        if x_index is None:
            x_index = int(self.params['source_x'])

        x_idx = np.clip(x_index, 0, self.nx - 1)
        surface_y_idx = int(np.mean(self.surface_y))

        depth_values = []
        concentration_values = []

        for y_idx in range(min(surface_y_idx, self.ny - 1), -1, -1):
            depth = (surface_y_idx - y_idx) * self.dy
            conc = self.c[y_idx, x_idx]
        ...
        if valid_indices:
            ...
            concentration_values = [concentration_values[i] for i in valid_indices]

        return depth_values, concentration_values
